...
public class StartLoginActivityTest {
    ...
    public void startLoginActivityTest() {
        ViewInteraction textView = onView(
                allOf(withId(R.id.tv_start_login), withText("登录"),
                        childAtPosition(
                                childAtPosition(
        ...
        ViewInteraction textView2 = onView(
                allOf(withId(R.id.tv_start_register), withText("注册"),
                        childAtPosition(
                                childAtPosition(
                                        IsInstanceOf.<View>instanceOf(android.widget.ScrollView.class),
                                        0),
                                2),
                        isDisplayed()));
        textView2.check(matches(withText("注册")));

        ViewInteraction textView3 = onView(
                allOf(withId(R.id.tv_tourist_login), withText("进去逛逛"),
                        childAtPosition(
                                childAtPosition(
                                        IsInstanceOf.<View>instanceOf(android.widget.ScrollView.class),
                                        0),
                                4),
                        isDisplayed()));
        textView3.check(matches(withText("进去逛逛")));
