...
/**
 * Created by gibxin on 2016/2/12.
 */
public class CircleImageView extends ImageView{
    ...
    // 默认圆角大小，默认为0，表示为圆形
    private static final int DEFAULT_RADIU = 0;
    ...
    public CircleImageView(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        // 获取圆角大小,默认为0，表示为圆形，如果大于0，表示为圆角矩形
        mRadiu = a.getDimensionPixelSize(R.styleable.CircleImageView_radiu, DEFAULT_RADIU);
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        {
            canvas.drawRoundRect(mRect, mRadiu, mRadiu, mBitmapPaint);
        }
    }
    ...
    public void setCornerRadiu(int radiu)
    {
        if (radiu == mRadiu)
            return;
        mRadiu = radiu;
    ...
    /**
     * 设置边界是否被覆盖，false表示显示边界, 默认为不覆盖，即false
     *
     * @param borderOverlay
     */
    ...
    /**
     * 这个函数很关键，进行图片画笔边界画笔(Paint)一些重绘参数初始化： 构建渲染器BitmapShader用Bitmap来填充绘制区域,设置样式以及内外圆半径计算等，
    ...
    private void updateShaderMatrix()
    {
        ...
        // 这里不好理解 这个不等式也就是(mBitmapWidth / mDrawableRect.width()) > (mBitmapHeight /
        // mDrawableRect.height())
        // 取最小的缩放比例
